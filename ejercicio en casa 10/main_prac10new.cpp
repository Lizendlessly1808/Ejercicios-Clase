...
visual studio 2015
ejercicio que consiste en la implementacion de rueditas al carrito
además, al llegar al final del camino el varrito irá en reversa y al llegar al otro
extremo del camino, avanzara el carrito,a si en un loop infinito
además, las llantas rotan dependiendo de la direccion el carrito
*/
...
float llanta_rotada = 0.0f;
bool llanta_animacion = false;
	...
	glPushMatrix();
		...
		glPushMatrix();		
			...
			glPushMatrix();
				...
				glRotatef(llanta_rotada, 1.0, 0.0, 0.0);
...
void animacion()
{
	...
	if(g_fanimacion)
	{
		if (llanta_animacion == false && movKit < 130)
		{
			movKit += 0.5;
			llanta_rotada += 1.0;

			if (movKit == 129)
				llanta_animacion = true;

		}

		if (llanta_animacion == true && movKit >- 130)
		{
			movKit -= 0.5;
			llanta_rotada -= 1.0;

			if (movKit == -129)
				llanta_animacion = false;
