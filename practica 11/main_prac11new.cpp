...
practica #11
descripcion:
en base al coddigo proporcionado se le agrego movimiento en las articulaciones de los brazos, cabeza y de ambas piernas al monito
junto con desplazamientos y movimientos de camara que ya venian predefinidos

manual de uso:
teclas:
c: movimiento de cabeza hacia izquierda
C: movimiento de cabeza hacia derecha

v: movimiento de brazo izquiero hacia enfrente
V: movimiento de brazo izquiero hacia atras

b: movimiento de pierna izquierda hacia atras
B: movimiento de pierna izquierda hacia adelante

n: movimiento de pierna derecha hacia atras
N: movimiento de pierna derecha hacia adelante

m:movimiento de brazo derecho hacia enfrente
M:movimiento de brazo derecho hacia atras

y: movimiento de camara hacia enfrente
h:movimiento de camara hacia atras
g:movimiento de camara hacia izquierda
j:movimiento de camara hacia derecha
*/
...
int i_max_steps = 100;//se aumenta para que el movimiento se vea mejor
int i_curr_steps = 0;
...
typedef struct _frame
{
	...
	float incX;		//Variable para IncrementoX lo calcula
	float incY;		//Variable para IncrementoY
	float incZ;		//Variable para IncrementoZ
	float rotRodIzq;
	float rotRodDer; // se agrega(1)
	float rotRodDerInc;
	float rotInc;
	float giroMonito;
	float giroMonitoInc;
...
void saveFrame ( void )
{
	...
	KeyFrame[FrameIndex].rotRodDer = rotRodDer;//(se agrega 2)
...
void resetElements( void )
{
	...
	rotRodDer = KeyFrame[0].rotRodDer;//Se agrega(3)
...
void interpolation ( void )
{
	...
	KeyFrame[playIndex].rotRodDerInc = (KeyFrame[playIndex + 1].rotRodDer - KeyFrame[playIndex].rotRodDer) / i_max_steps; //se agrega(4)

	KeyFrame[playIndex].rotBrazoDer = (KeyFrame[playIndex + 1].rotBrazoDer - KeyFrame[playIndex].rotBrazoDer) / i_max_steps;
	KeyFrame[playIndex].rotBrazoIzq = (KeyFrame[playIndex + 1].rotBrazoIzq - KeyFrame[playIndex].rotBrazoIzq) / i_max_steps;
	KeyFrame[playIndex].rotCabeza= (KeyFrame[playIndex + 1].rotCabeza - KeyFrame[playIndex].rotCabeza) / i_max_steps;
}
	...
	glPushMatrix();//Pecho
		...
		glPushMatrix(); //Brazo derecho-->
			...
			glPushMatrix();
				...
				glRotatef(-45 + rotBrazoDer, 0, 1, 0);
		...
		glPushMatrix(); //Brazo izquierdo <--
			...
			glPushMatrix();
				glTranslatef(-0.25, 0, 0);
				glRotatef(45 + rotBrazoIzq, 0, 1, 0);
				//glRotatef(45 + rotBrazoIzq, 0, 1, 0);
				//glRotatef(25, 0, 0, 1);
				//glRotatef(25, 1, 0, 0); 
				glTranslatef(-0.75, 0, 0);
		...
		glPushMatrix();//Cintura
			...
			//PIERNA
				...
				glPushMatrix();
					glTranslatef(0, -0.5, 0);
					glRotatef(15+rotRodDer, 1, 0, 0);//se agrega (5)
...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	for(int i=0; i<MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].rotRodDer = 0; //paso(8)
		KeyFrame[i].rotRodDerInc = 0; //paso(8)
